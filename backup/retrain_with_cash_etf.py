...
class EnhancedETFSwitchingStrategy:
    ...
    def create_targets(self, features_df, price_df):
        """Create target labels for best ETF to hold"""
        ...
        for i in range(len(features_df)):
            ...
            try:
                ...
                targets.append('VMOT')  # Default to cash if date not found
                continue
            ...
            if date_idx + 5 < len(price_df):
                ...
                for symbol in symbols:
                    ...
                    if pd.notna(current_price) and pd.notna(future_price) and current_price > 0:
                        ...
                        future_returns[symbol] = 0
                
                # Choose the ETF with highest future return
                if future_returns:
                    best_etf = max(future_returns.keys(), key=lambda x: future_returns[x])
                    
                    # Risk management: if best return is negative and VMOT available, choose VMOT
                    if future_returns[best_etf] < -0.02 and 'VMOT' in future_returns:  # -2% threshold
                        best_etf = 'VMOT'
                    
                    targets.append(best_etf)
                else:
                    targets.append('VMOT')  # Default to cash
            else:
                targets.append('VMOT')  # Default to cash for end of data
